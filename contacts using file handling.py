...
FILE_NAME = r"C:\Users\T.kaushik ram\OneDrive\Desktop\PYTHON VS CODES\.ipynb_checkpoints\contacts.csv"
# adding new contact
...
def save_to_csv():
    with open("contacts.csv", "w", newline="") as file:
        ...
        writer.writerow(["Name", "Mobile"])   # header
...
if __name__=="__main__":
    ...
    load_from_csv()
    save_to_csv()
    print("Welcome to the conatcts management")
    while(True):
        print("Select your operatoin \n 1>Add contact \n 2.Update contact \n 3.Delete contact \n 4.Search mobile number by name \n 5.see all contacts \n 6.Exit")
        choice = int(input("Enter your choice: "))
        if(choice==1):
            print("1.add contacts")
            name = input("Enter the name: ").strip()
            number = int(input("Enter the number: "))
            res = add_contact(name=name, mobile=number)
            save_to_csv()
            print(res)
        elif(choice==2):
            ...
            name = input("Enter the name: ").strip()
            number = int(input("Enter the number: "))
            res = update_contact(name=name, mobile=number)
            save_to_csv()
            print(res)
        elif(choice==3):
            ...
            name = input("Enter the name: ").strip()
            res = get_mobile_number(name=name,mobile=number)
            save_to_csv()
            print(res)
        elif(choice==5):
